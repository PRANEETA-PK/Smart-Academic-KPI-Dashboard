...
import { FolderGit2, Plus, Github, ExternalLink, Code, Trash2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function ProjectPortfolio({ student, onProjectAdded }) {
  const { token } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("/api/students/projects", formData);
  ...
  const handleDelete = async (projectId) => {
    ...
    try {
      const res = await axios.delete(`/api/students/projects/${projectId}`);
      if (res.data) {
        onProjectAdded(res.data);
      }
    } catch (err) {
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription className="hidden">Submit your completed project details.</DialogDescription>
            </DialogHeader>
                  ...
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                        proj.approvalStatus === 'Approved' ? 'bg-success/10 text-success' :
                        proj.approvalStatus === 'Rejected' ? 'bg-destructive/10 text-destructive border border-destructive/20' :
                        'bg-warning/10 text-warning animate-pulse'
                      }`}>
                        {proj.approvalStatus || 'Pending'}
                      </span>
                    </div>
                    <button onClick={() => handleDelete(proj._id)} className="text-muted-foreground hover:text-destructive transition-colors" title="Delete Project">
