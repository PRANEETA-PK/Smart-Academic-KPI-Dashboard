...
const WhatIfCalculator = ({ semesters, currentCGPA }) => {
    ...
    const getRequiredSGPA = (targetCGPA) => {
    ...
    return (
        <div className="rounded-xl border border-border bg-card p-5 shadow-card hover:shadow-glow transition-shadow animate-fade-in">
            ...
            <div className="mb-6 rounded-lg bg-accent p-4 text-center shadow-glow-soft">
                ...
                <p className={`font-display text-4xl font-bold ${predictedCGPA >= 9 ? "text-success" : predictedCGPA >= 7 ? "text-primary" : "text-destructive"
                    }`}>
                ...
                {targets.map((t) => {
                    ...
                    return (
                        <div key={t.label} className="flex items-center justify-between rounded-lg border border-border px-4 py-3">
                            ...
                            <span className={`text-sm font-bold ${achievable ? (required <= simulatedSGPA[0] ? "text-success" : "text-primary") : "text-muted-foreground"
                                }`}>
