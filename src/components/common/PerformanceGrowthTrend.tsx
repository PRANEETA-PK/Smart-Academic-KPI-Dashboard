...
const PerformanceGrowthTrend: React.FC<Props> = ({ semesters }) => {
  ...
  const getInsight = () => {
    if (latestChange > 0)
      return { text: `Your performance improved by ${latestChange}% compared to last semester.`, type: "success" as const };
    if (latestChange < 0)
      return { text: `Your performance dropped by ${Math.abs(latestChange)}% compared to last semester.`, type: "destructive" as const };
    return { text: "Your performance is stable compared to last semester.", type: "primary" as const };
  };
    ...
    <div className="rounded-xl border border-border bg-card p-5 shadow-card hover:shadow-glow transition-shadow animate-fade-in">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {trendData.slice(1).map((d) => (
          <div key={d.semesterName} className={`rounded-full px-3 py-1 text-xs font-medium ${
            d.isImproved ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"
          }`}>
